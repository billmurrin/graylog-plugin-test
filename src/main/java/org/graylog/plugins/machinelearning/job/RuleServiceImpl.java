...
import org.graylog.plugins.machinelearning.job.rest.models.requests.AddRuleRequest;
import org.graylog.plugins.machinelearning.job.rest.models.requests.UpdateRuleRequest;
...
public class RuleServiceImpl implements RuleService {
	...
	public Rule update(String name, Rule rule) {		
		...
		if (rule instanceof RuleImpl) {
			final RuleImpl ruleImpl = (RuleImpl) rule;
			LOG.debug("updated job: " + ruleImpl);
			final Set<ConstraintViolation<RuleImpl>> violations = validator.validate(ruleImpl);
			if (violations.isEmpty()) {
	...
	public Rule setCurrentAlertId(Rule rule, String currentAlertId){
		if (rule instanceof RuleImpl) {
			final RuleImpl ruleImpl = (RuleImpl) rule;
			LOG.debug("set current alert id for job: " + ruleImpl);
			final Set<ConstraintViolation<RuleImpl>> violations = validator.validate(ruleImpl);
			if (violations.isEmpty()) {
